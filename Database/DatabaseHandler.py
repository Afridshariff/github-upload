...
class DatabaseHandler:
  ...
  def connect(self):
    if(self.connection==None):
      if(os.path.exists("Database\\RagaDetection_DB.db")):
        self.connection = sqlite3.connect('Database\\RagaDetection_DB.db')
      else:
        self.connection = self.create_DB()
  
  def create_DB(self):
    connection = sqlite3.connect("Database\\RagaDetection_DB.db")
    cursor = connection.cursor()
    cursor.executescript('''CREATE TABLE Swara (
      'id' int,
      's_name' char,
      'vikruti' int,
      'name' varchar(255),
      ...
      'madhyama_name' varchar(255),
      'madhyama_vikruti' int,
  ...
  def getEntityByAttr(entity, attribute, value):
    return
  
  def getEntityById(entity, id):
    return