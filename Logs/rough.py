...
raga_dict = dict()
binaries = list()
with open(r"C:\Users\1000272222\Desktop\janya.csv", "r") as f:
    a = f.readline()
    while a:
        temp = a.strip().split(",")
        tempstr = ""
        tempnotes = temp[1].split()
        tempnotes2 = temp[2].split()
        if (int(temp[3]) == 28):
            __ = True
        ...
        if set(tempnotes) != set(tempnotes2):
            a = f.readline()
            continue
        ...
        binaries.append(binary_string)
        if(binary_string in raga_dict.keys()):
            a = f.readline()
            continue
        raga_dict[binary_string] = {"Name": temp[0].capitalize(),
                       "Melakartha":int(temp[3]),
                       "Carnatic Notes": [get_key(i) for i in sorted(set(tempnotes))],
                       "Steps": sorted(set(tempnotes))}
        a = f.readline()

with open("derived_ragas.bin", "w") as f:
